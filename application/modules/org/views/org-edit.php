                    ...
                    <div class="page-title-box">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        <form method="POST" action="<?= base_url('org/edit/') . encrypt_url($orgs['ID']); ?>">
                                            <?= csrf(); ?>
                                            <div class="row mb-3">
                                                <label for="example-text-input" class="col-sm-2 offset-sm-1 col-form-label">Organisasi</label>
                                                <div class="col-sm-8">
                                                    <?php foreach ($orgz as $org) { ?>
                                                        <p class="mt-1 mb-0 text-muted"><strong><?= $org['SHORT_ORG']; ?></strong></p>
                                                    <?php } ?>
                                                    <p class="mt-1 mb-0 text-primary"><strong><?= $orgs['SHORT_ORG']; ?></strong></p>
                                                </div>
                                            </div>
                                            <div class="row mb-3">
                                                <label for="example-text-input" class="col-sm-2 offset-sm-1 col-form-label">Short Organisasi</label>
                                                <div class="col-sm-4">
                                                    <input type="text" name="short" class="form-control <?= form_error('short', 'is-invalid '); ?>" placeholder="Short Organisasi" value="<?= $orgs['SHORT_ORG']; ?>">
                                            ...
                                            <div class="row mb-3">
                                                ...
                                                <div class="col-sm-8">
                                                    <input type="text" name="long" class="form-control <?= form_error('long', 'is-invalid '); ?>" placeholder="Long Organisasi" value="<?= $orgs['LONG_ORG']; ?>">
                                                    <?= form_error('long', '<div class="invalid-feedback">', '</div>'); ?>
                                                </div>
                                            </div>
                                            <div class="row mb-3">
                                                <label for="example-text-input" class="col-sm-2 offset-sm-1 col-form-label">Status</label>
                                                <div class="col-sm-2">
                                                    <select name="status" class="form-control status <?= form_error('status', 'is-invalid '); ?>">
                                                        <option></option>
                                                        <option value="1" <?php echo ($orgs['IS_ACTIVE'] == 1) ? " selected=' selected'" : "" ?>>AKTIF</option>
                                                        <option value="0" <?php echo ($orgs['IS_ACTIVE'] == 0) ? " selected=' selected'" : "" ?>>TIDAK AKTIF</option>
                                                    </select>
