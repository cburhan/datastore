                    ...
                    <div class="page-title-box">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        <form method="POST" action="<?= base_url('kit_pipagas/edit/') . encrypt_url($pipa['ID']); ?>">
                                            <?= csrf(); ?>
                                            <div class="row mb-3">
                                                <label for="example-text-input" class="col-sm-2 offset-sm-2 col-form-label">Pipa Gas</label>
                                                <div class="col-sm-6">
                                                    <input type="text" name="pipa" class="form-control <?= form_error('pipa', 'is-invalid '); ?>" placeholder="Pipa Gas" value="<?= $pipa['PIPA_GAS']; ?>">
                                                    <?= form_error('pipa', '<div class="invalid-feedback">', '</div>'); ?>
                                                </div>
                                            </div>
                                            <div class="row mb-3">
                                                ...
                                                <div class="col-sm-2">
                                                    <select name="status" class="form-control status <?= form_error('status', 'is-invalid '); ?>">
                                                        <option></option>
                                                        <option value="1" <?= ($pipa['IS_ACTIVE'] == 1) ? " selected=' selected'" : "" ?>>AKTIF</option>
                                                        <option value="0" <?= ($pipa['IS_ACTIVE'] == 0) ? " selected=' selected'" : "" ?>>TIDAK AKTIF</option>
                                                    </select>
                                            ...
                                            <div class="mb-3 row">
                                                <div class="col-sm-10 offset-sm-1 text-center">
                                                    ...
                                                    <button class="btn btn-sm btn-outline-warning waves-effect waves-light" onclick="window.location.href = '<?= base_url('kit_pipagas'); ?>';" type="button"><i class="ion ion-md-close me-1"></i>Batal</button>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
