...
class Kit_demand_tahunan extends MY_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        check_permission();
        $this->load->model('Kit_demand_tahunan_model');
        $this->load->model('Kit_demand_tahunan_upload_model');
    ...
    public function index()
    {
        ...
        $this->load->view('kit-demand-tahunan-view', $data);
    }

    public function get_data()
    {
        $fetch_data = $this->Kit_demand_tahunan_model->make_datatables();
        $data = array();
        ...
        foreach ($fetch_data as $row) {
            ...
            if (check_button('edit') > 0) {
                $edit = '<a href="' . base_url('kit_demand_tahunan/edit/') . encrypt_url($row->ID) . '" class="btn btn-warning btn-sm waves-effect"><i class="ion ion-md-color-filter"></i></a>';
            }

            if (check_button('delete') > 0) {
                $del = '<a href="' . base_url('kit_demand_tahunan/delete/') . encrypt_url($row->ID) . '" class="btn btn-danger btn-sm waves-effect" onclick="return confirmDelete()"><i class="ion ion-md-trash"></i></a>';
            }
        ...
        $output = array(
            ...
            'recordsTotal'      => $this->Kit_demand_tahunan_model->get_all_data(),
            'recordsFiltered'   => $this->Kit_demand_tahunan_model->get_filtered_data(),
            'data'              => $data
        );
    ...
    public function edit($id)
    {
        ...
        $data['demand']    = $this->Kit_demand_tahunan_model->getKitDemandTahunanById($id);
        ...
        if ($this->form_validation->run() == FALSE) {
            if ($data['demand'] != NULL) {
                $this->load->view('kit-demand-tahunan-edit', $data);
            } else {
            ...
            $this->Kit_demand_tahunan_model->editKitDemandTahunan($data, $id);
    ...
    public function delete($id)
    {
        ...
        $demand = $this->Kit_demand_tahunan_model->getKitDemandTahunanById($id);
        $delete = $this->Kit_demand_tahunan_model->deleteKitDemandTahunan($id);
        if ($delete) {
            $flash = '<div class="alert alert-success alert-dismissible bg-success text-white border-0" role="alert">
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        <strong>Sukses!</strong> Data Demand Pembangkit Tahunan berhasil dihapus.
                        </div>';
            $this->session->set_flashdata('flash', $flash);
            $ket = 'Menghapus data <strong>Demand Pembangkit Tahun</strong> <strong>' . $demand['TAHUN'] . '</strong> dengan ID #<strong>' . $id . '</strong>';
            ...
            $flash = '<div class="alert alert-danger alert-dismissible bg-danger text-white border-0" role="alert">
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        <strong>Gagal!</strong> Data Demand Pembangkit Tahunan masih digunakan.
                        </div>';
            $this->session->set_flashdata('flash', $flash);
        }
        redirect('kit_demand_tahunan');
    ...
    public function upload()
    {
        ...
        $data['file'] = $this->Kit_demand_tahunan_upload_model->file_uploaded()->result_array();

        $this->load->view('kit-demand-tahunan-upload', $data);
    }
    ...
    public function do_upload()
    {
        ...
        if ($upload_file) {
            $config['upload_path']      = './public/upload_file/demand_tahunan';
            $config['allowed_types']    = 'xls|xlsx';
            $config['remove_spaces']    = true;

            $time = date('Y-m-d H:i:s');
            $time_name = date('YmdHis', strtotime($time));
            $config['file_name']        = $time_name . '_DEMAND_TAHUNAN';
            ...
            if ($this->upload->do_upload('excel')) {
                ...
                $add = $this->Kit_demand_tahunan_upload_model->add_upload($data);
                $flash = '<div class="alert alert-success alert-dismissible bg-success text-white border-0" role="alert">
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                            <strong>Sukses!</strong> File berhasil diupload.
                            </div>';
                $this->session->set_flashdata('flash', $flash);
                redirect('kit_demand_tahunan/detail_upload/' . encrypt_url($add));
            } else {
                ...
                $this->session->set_flashdata('flash', $flash);
                redirect('kit_demand_tahunan/upload');
            }
        } else {
            $flash = '<div class="alert alert-danger alert-dismissible bg-danger text-white border-0" role="alert">
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    ...
    public function download_template()
    {
        ...
        $sheet->setTitle('DEMAND_TAHUNAN');

        $filename = 'DEMAND_TAHUNAN_TEMPLATE.xlsx';
    ...
    public function detail_upload($file_id)
    {
        ...
        $file = $this->Kit_demand_tahunan_upload_model->get_file_by_id($file_id);
        if ($file != NULL) {
            if ($file['STATUS'] == 'EXE') {
                $data['log'] = $this->Kit_demand_tahunan_upload_model->get_log_upload($file_id)->result_array();
            }
            $path = 'public/upload_file/demand_tahunan/' . $file['FILE'];
            ...
            $this->load->view('kit-demand-tahunan-detail-upload', $data);
        } else {
            $this->load->view('myerror/error404', $data);
        }
    }
    ...
    public function exe_upload($file_id)
    {
        ...
        foreach ($object->getWorksheetIterator() as $worksheet) {
            ...
            for ($row = 2; $row <= $highestRow; $row++) {
                ...
                $p = $this->Kit_demand_tahunan_upload_model->get_not_kit($kode_pembangkit, $tahun);
                if ($p->num_rows() == 0) {
                    $data_insert = array(
                    ...
                    $this->Kit_demand_tahunan_model->addKitDemandTahunan($data_insert);
                    $tipe = "INSERT";
                } else {
                    $data_update = array(
                        'KODE_PEMBANGKIT'       => $kode_pembangkit,
                    ...
                    $this->Kit_demand_tahunan_model->editKitDemandTahunanByKode($data_update, $kode_pembangkit, $tahun);
                    $tipe = "EXIST";
                }
                $data_log = array(
                    'UPLOAD_ID'             => $file_id,
                ...
                $this->Kit_demand_tahunan_upload_model->insert_log($data_log);
            }
        }
        ...
        $this->Kit_demand_tahunan_upload_model->edit_upload($data_file, $file_id);
        $flash = '<div class="alert alert-success alert-dismissible bg-success text-white border-0" role="alert">
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    <strong>Sukses!</strong> Data berhasil dieksekusi.
                    </div>';
        $this->session->set_flashdata('flash', $flash);
        redirect('kit_demand_tahunan/detail_upload/' . encrypt_url($file_id));
    }

    public function publish()
    {
        $demand     = $this->Kit_demand_tahunan_model->getKitDemandTahunan()->result_array();
        if ($demand != NULL) {
            ...
            $db         = api()['DATABASE'];
            $schema     = api()['SCHEMA'];
            $table      = 'DATASTORE_MANUAL_KITDEMANDTAHUNAN';
            ...
            if ($token) {
                foreach ($demand as $d) {
                    $data[] = [
                        ...
                        'SOURCE'                => 'DATASTORE_MANUAL_KITDEMANDTAHUNAN'
                    ];
                }
            ...
            <strong>Sukses!</strong> Data berhasil dipublish di Snowflake!
                ...
                $this->session->set_flashdata('flash', $flash);
                $ket = 'Mempublish data <strong>Demand Pembangkit Tahunan</strong> ke <strong>Snowflake</strong>';
